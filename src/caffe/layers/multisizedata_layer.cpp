...
void MultiSizeDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    top_shapes[item_id] = this->data_transformer_->InferBlobShape(datum[item_id]);
  ...
  sdH = sqrt(sdH/batchSize)/1.5;//tighten distribution
  sdW = sqrt(sdW/batchSize)/1.5;
  ...
  top_shape[2]=max(distriburionH(randgen),10.0f);
  top_shape[3]=max(distriburionW(randgen),10.0f);
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    CHECK_EQ(datums[item_id].channels(),1) <<"MultiSizeDataLayer only implemented from single channel images.";
      const string& data = datum[item_id].data();
      const int datum_height = datum[item_id].height();
      const int datum_width = datum[item_id].width();
        ...
        for (int h = 0; h < height; ++h) {
          for (int w = 0; w < width; ++w) {
            ...
            if (has_uint8) {
              ...
              datum_element = datum[item_id].float_data(data_index);
            }
            fitted.at<Dtype>(h,w) = datum_element;
          }
        }
      }
    ...
    this->data_transformer_->Transform(fitted, &(this->transformed_data_));
