...
LayerParameter TPPLayer<Dtype>::GetPoolingParam(const int pyramid_level,
  ...
  //too small image case
  while (pad_w>=kernel_w)
  {
      //std::cout<<"bottom_w:"<<bottom_w<<" pyramid_level:"<<pyramid_level<<" kernel_w:"<<kernel_w<<" remainder_w:"<<remainder_w<<" pad_w:"<<pad_w<<" stride_w:"<<stride_w<<std::endl;
      stride_w-=1;
      remainder_w = ( kernel_w*pyramid_level - ((kernel_w-stride_w)*(pyramid_level-1)) ) - bottom_w;
      pad_w = (remainder_w + 1) / 2;
      //std::cout<<"   new  remainder_w:"<<remainder_w<<" pad_w:"<<pad_w<<" stride_w:"<<stride_w<<std::endl;

      if (remainder_w<0)
      {
          kernel_w+=1;
          remainder_w = kernel_w * pyramid_level - bottom_w;
          pad_w = (remainder_w + 1) / 2;
          stride_w=kernel_w;
      }
  }
