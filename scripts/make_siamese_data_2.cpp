...
string read_image(string image_file, bool match) {
...
void convert_dataset(const vector<string>& image_filenames, const vector<string>& labels,
  ...
  while (toWrite.size()>0) {
        ...
        string value = read_image(image_filenames[im1],match);
        string value2 = read_image(image_filenames[im2],match);
...
int main(int argc, char** argv) {
  if (argc != 5) {
    printf("This script converts the dataset to the leveldb format used\n"
           ...
           "output1_db_file output2_db_file\n"
           );
  } else {
